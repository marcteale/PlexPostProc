...
if [ ! -f "$1" ]; then
    msg="Input file $1 does not exist"
    logger -t PlexPostProc.sh $msg
    echo $msg
    exit 1
fi

logger -t PlexPostProc.sh Start transcode process for $1
...
if [[ $ENCODER == "handbrake" ]]; then
    ...
    logger -t PlexPostProc.sh "Invalid value \"$ENCODER\" for ENCODER.  Failing."
    exit 1
fi
...
while [ true ] ; do
    if ls "$TMPFOLDER/"*".ppplock" 1> /dev/null 2>&1; then
        ...
        if  [[ $timeout_counter -lt 119 ]]; then
            logger -t PlexPostProc.sh "It looks like all scripts are done running, exiting."
        fi
        break
    fi
done
